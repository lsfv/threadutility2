...
namespace Function
{
    ...
    public class Utils
    {
        ...
        /// <returns></returns>
        public static DataTable GetTableFEnum(Type p_EnumType, string columnname, string columnvalue)
        {
        ...
        /// <returns></returns>
        public static string MD5NET(string md5str)
        {
        ...
        /// <returns></returns>
        public static string Decode(string Str, string CodeType)
        {
            string UrlDecode = HttpUtility.UrlDecode(Str, System.Text.Encoding.GetEncoding(CodeType));
            return UrlDecode;
        }
        /// <summary>
        /// 对中文编码以并GB2312编码输出
        /// </summary>
        public static string Encode(string Str, string CodeType)
        {
            string UrlEncode = HttpUtility.UrlEncode(Str, System.Text.Encoding.GetEncoding(CodeType));
            return UrlEncode;
        }


        /// <summary>
        /// 返回当前请求地址的运用程序的根目录，带“/”,如:http://xx/;http://xx:801;http://虚拟目录/
        /// </summary>
        /// <returns></returns>
        public static string GetWebAPPPath()
        {
            string strServer = "http://" + HttpContext.Current.Request.ServerVariables["SERVER_NAME"];
        ...
        public static string picadd(string altstr,string content)
        {
            string picadd = "";
            MatchCollection mc = Regex.Matches(content, @"(?is)<img\b(?=(?:(?!alt=|>).)*alt=(['""]?)(?<alt>[^'""\s>]*)\1[^>]*>)(?:(?!src=).)*src=(['""]?)(?<src>[^'""\s>]*)\2[^>]*>");
            foreach (Match m in mc)
            {
                if (m.Groups["alt"].Value == altstr)
                {
                    picadd = m.Groups["src"].Value;
                    //匹配第一个
                    break;
                }
            }
        ...
        //得到网站虚拟目录路径。
        public static string pathvirth()
        {
            string va = "";
            if ("" == va)
            {
